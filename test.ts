...
function sortVector(points: Point3d[]): Point3d[] {
    return points.sort((a, b) =>
        distance(a, [0, 0, 0]) - distance(b, [0, 0, 0])
    );
}
...
function getSplinePoints(
    points: Point3d[],
    i: number,
): [Point3d, Point3d, Point3d, Point3d] | undefined {
...
const casteljau = (
    accuracy: number,
    [p1, p2, p3, p4]: [Point3d, Point3d, Point3d, Point3d],
): Point3d => {
...
class Vector {
    ...
    intersectPlane(plane: Plane): Point3d {
        ...
        const t = (normal.x.magnitude * (point[0] - this.point[0]) +
            normal.y.magnitude * (point[1] - this.point[1]) +
            normal.z.magnitude * (point[2] - this.point[2])) /
...
const sphere = (
    ...
    return new Array(layers).fill(0).flatMap((_, i) => {
        ...
        return circle(
            r,
            circleDetail,
            center.move([center.point[0], center.point[1], z]),
        );
    });
};
...
const draw = () => {
    ...
    document.querySelector("#fps")!.innerHTML =
        (totalFrames / ((now - start) / 1000)).toFixed(2);
};
