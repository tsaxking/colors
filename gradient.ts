...
class Gradient {
    ...
    static curve(
        ...
        return new Gradient(
            ...Array(frames).fill(0).map((_, i, arr) => {
                const x = i / frames;
                const y = fx(x);
    ...
    public static drawGrid(ctx: CanvasRenderingContext2D) {
        // draw a cube
        const cubePoints: [number, number, string][] = [
            [0, 1, "black"],
            [2, 0, "red"],
            [4, 0, "black"],
            [2, 2, "black"],
